...
private:
    ...
    uint64_t round16(uint64_t input, uint64_t* sub_key);

public:
    uint64_t encrypt(uint64_t p, uint64_t k);
    uint64_t decrypt(uint64_t c, uint64_t k);
...
char des::iteration_shift[] = {
    1,  1,  2,  2,  2,  2,  2,  2,  1,  2,  2,  2,  2,  2,  2,  1
};


uint64_t des::encrypt(uint64_t p, uint64_t k) {
    int i, j;
    ...
    for (i = 0; i < 56; i++) {
        ...
        permuted_choice_1 <<= 1;
        permuted_choice_1 |= (k >> (64 - PC1[i])) & LB64_MASK;
    ...
    return round16(p, sub_key);

};

uint64_t des::decrypt(uint64_t c, uint64_t k) {
    int i, j;
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (j = 0; j < 48; j++) {
            ...
            sub_key[15-i] <<= 1;
            sub_key[15-i] |= (permuted_choice_2 >> (56 - PC2[j])) & LB64_MASK;

        }

    }

    return round16(c, sub_key);

};

uint64_t des::round16(uint64_t input, uint64_t* sub_key) {
    int i, j;
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            s_output <<= 4;
            s_output |= (uint32_t)(S[j][16 * row + col] & 0x0f);
...
int main()
{
    ...
    uint64_t plaintext = 0x0000000000000001;
    uint64_t key = 0x0000000000000000;
    uint64_t ciphertext = d.encrypt(plaintext, key);

    cout << hex;
    cout << "Ciphertext:" << ciphertext << endl;
    cout << hex;
    cout << "Plaintext:" << d.decrypt(ciphertext, key) << endl;
    return 0;
}
